...
dl.apply_field(
    densify, field_name="matrix", new_field_name="matrix", progress_bar="Densify"
)

print(dl)
label2idx = (
    getattr(dl, "ner_vocab") if hasattr(dl, "ner_vocab") else getattr(dl, "label2idx")
)
...
optimizer = torch.optim.AdamW(
    [
        ...
        {
            "params": non_pretrain_ln_params,
            "lr": args.lr * non_ptm_lr_ratio,
            "weight_decay": 0,
        },
        {
            "params": non_pretrain_params,
            "lr": args.lr * non_ptm_lr_ratio,
...
metrics = {
    "f": NERMetric(
        matrix_segs=matrix_segs, ent_thres=ent_thres, allow_nested=allow_nested
    )
}
...
trainer.run(
    num_train_batch_per_epoch=-1, num_eval_batch_per_dl=-1, num_eval_sanity_batch=1
)
fitlog.finish()
