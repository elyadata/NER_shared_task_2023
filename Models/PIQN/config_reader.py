...
def process_configs(target, arg_parser):
    ...
    for run_args, _run_config, _run_repeat in _yield_configs(arg_parser, args):
        ...
        if run_args.debug:
            target(run_args)
        while not run_args.cpu and (
            len(gpu_queue) == 0 or len(gpu_queue) < run_args.world_size
        ):
            gpu_queue = []
                ...
                try:
                    ...
                    #     gpu_queue.append(index)
                        ...
                        if meminfo.free / 1024 / 1024 > 12000:
                            gpu_queue.extend(
                                [index] * (meminfo.free // 1024 // 1024 // 12000)
                            )
                    else:
                        if meminfo.free / 1024 / 1024 > 24500:
                            gpu_queue.extend(
                                [index] * (meminfo.free // 1024 // 1024 // 24000)
                            )
        ...
        # GPU Training
            ...
            if run_args.world_size != -1:
                ...
                print("Using devices: ", os.environ["CUDA_VISIBLE_DEVICES"])
        ...
        for run_repeat, run_config in config:
            ...
            if label.startswith("batch_eval_flag"):
                ...
                for dirpath, dirnames, filenames in sorted(
                    os.walk(save_path), key=lambda x: x[0]
                ):
                    if (
                        "_model" in dirpath
                        and "xlmr" not in dirpath
                        and "/bn/" not in dirpath
                    ):
                        print(dirpath)
                        if label == "batch_eval_flag_multilingual":
                            ...
                            for lan in lans:
                                args_path = (
                                    "/".join(dirpath.split("/")[:-1]) + "/args.json"
                                )
                                ...
                                run_args.dataset_path = (
                                    args_dict["train_path"]
                                    .replace("train", "dev")
                                    .replace("/" + orig_name, "/" + lan)
                                )
                                ...
                                if run_args.eval_batch_size == -1:
                                    run_args.eval_batch_size = args_dict[
                                        "eval_batch_size"
                                    ]
                                ...
                                run_args.char_lstm_layers = args_dict[
                                    "char_lstm_layers"
                                ]
                                ...
                                run_args.repeat_gt_entities = args_dict[
                                    "repeat_gt_entities"
                                ]
                                ...
                                run_args.match_boundary_weight = args_dict[
                                    "match_boundary_weight"
                                ]
                                run_args.match_class_weight = args_dict[
                                    "match_class_weight"
                                ]
                                run_args.loss_boundary_weight = args_dict[
                                    "loss_boundary_weight"
                                ]
                                run_args.loss_class_weight = args_dict[
                                    "loss_class_weight"
                                ]
                                run_args.match_solver = args_dict["match_solver"]
                                run_args.share_query_pos = args_dict["share_query_pos"]
                                run_args.use_token_level_encoder = args_dict[
                                    "use_token_level_encoder"
                                ]
                                run_args.num_token_entity_encoderlayer = args_dict[
                                    "num_token_entity_encoderlayer"
                                ]
                                run_args.use_entity_attention = args_dict[
                                    "use_entity_attention"
                                ]

                                run_args.entity_queries_num = args_dict[
                                    "entity_queries_num"
                                ]
                                ...
                                if "word_mask_ent2ent" in args_dict:
                                    run_args.word_mask_ent2ent = args_dict[
                                        "word_mask_ent2ent"
                                    ]
                                    run_args.word_mask_ent2tok = args_dict[
                                        "word_mask_ent2tok"
                                    ]
                                    run_args.word_mask_tok2ent = args_dict[
                                        "word_mask_tok2ent"
                                    ]
                                    run_args.word_mask_entself = args_dict[
                                        "word_mask_entself"
                                    ]
                                else:
                                ...
                                run_args.entity_aware_attention = args_dict[
                                    "entity_aware_attention"
                                ]
                                run_args.entity_aware_selfout = args_dict[
                                    "entity_aware_selfout"
                                ]
                                run_args.entity_aware_intermediate = args_dict[
                                    "entity_aware_intermediate"
                                ]
                                run_args.entity_aware_output = args_dict[
                                    "entity_aware_output"
                                ]
                                run_args.use_entity_pos = args_dict["use_entity_pos"]
                                if "use_entity_common_embedding" in args_dict:
                                    run_args.use_entity_common_embedding = args_dict[
                                        "use_entity_common_embedding"
                                    ]
                                ...
                                if (
                                    run_args.cls_threshold == -1
                                    and run_args.boundary_threshold != -1
                                ):
                                ...
                                if (
                                    run_args.boundary_threshold == -1
                                    and run_args.cls_threshold != -1
                                ):
                                    for boundary_threshold in np.arange(0, 1, 0.1):
                                        run_args_instance = copy.deepcopy(run_args)
                                        run_args_instance.boundary_threshold = (
                                            boundary_threshold
                                        )
                                        run_args_list.append(run_args_instance)
                                ...
                                if (
                                    ...
                                    for cls_threshold in np.arange(0, 1, 0.1):
                                        for boundary_threshold in np.arange(0, 1, 0.1):
                                            run_args_instance = copy.deepcopy(run_args)
                                            run_args_instance.cls_threshold = (
                                                cls_threshold
                                            )
                                            run_args_instance.boundary_threshold = (
                                                boundary_threshold
                            ...
                            if "train_dev" in args_dict["train_path"]:
                                run_args.dataset_path = args_dict["train_path"].replace(
                                    "train_dev", "test"
                                )
                            else:
                                run_args.dataset_path = args_dict["train_path"].replace(
